...
from visualization.path import FloorPlanVisualizer, save_figure
...
class Test11Rooms:
	def test_complex_rooms(self,complex_floor_plan_fixture):
		...
		visualizer = FloorPlanVisualizer(floorPlan, ax)
...
def test_show_generated_floorPlan():
	...
	floorPlan = FloorPlan([],visualizer)
	cm = 1/2.54  # centimeters in inches
	visualizer.show()
	save_figure(ax, "Autogenerated_floor_plan")
	assert(isinstance(floorPlan, FloorPlan))
	assert(isinstance(ax, plt.Axes))
	assert(isinstance(fig, plt.Figure))
	floorPlan.generate(width=25, length=25, base_rooms=3,random_rooms=5,
					min_ratio=0.05, max_ratio=0.40, max_aspect_ratio=4.0, ax=ax)
	plt.show(block=True)