...
from tests.conftest import room_plan_11_rooms_random_concete_fixture

class TestRandomConcreteFixture:
	def test_returns_floor_plan(self, room_plan_11_rooms_random_concete_fixture):
		floor_plan = room_plan_11_rooms_random_concete_fixture()
		assert isinstance(floor_plan, FloorPlan)

	def test_wall_types_are_valid(self, room_plan_11_rooms_random_concete_fixture):
		floor_plan = room_plan_11_rooms_random_concete_fixture()
		for wall in floor_plan.walls:
			assert wall.wallType in [WallType.OUTER_WALL, WallType.DRYWALL, WallType.CONCRETE]

	def test_outer_walls_unchanged(self, room_plan_11_rooms_random_concete_fixture):
		floor_plan = room_plan_11_rooms_random_concete_fixture()
	...
	def test_drywall_probability(self, room_plan_11_rooms_random_concete_fixture):
